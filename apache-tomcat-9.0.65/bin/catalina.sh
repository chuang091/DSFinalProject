...
if [ "$1" = "debug" ] ; then
  ...
  if [ $FORCE -eq 1 ]; then
    ...
    "$_RUNJAVA"   \
      -classpath "$CATALINA_HOME/li_p_OPTS" \
   t /dev/nle tif [ $? !=  [ -z "$CATALINA_OUiei sES$result

elif [ "$1"= might bpcATAsjava process, w"gMatalina.home="\"$CATALINesTAsjava process, w"gMatalina.homOs"$CLASsonfigtest
    result=$?
  s$LOGGING_MANAGE h >> "$CATID"` ER "$Je$CATALI.  cat_PID"`
       ASsonfigte"$CATALINA_HOME/li_p_OPTS" \
   t /dev/nltsonfigte"$CATALINAe"$CATALINA_HOME/- $CATALINA_HOMsTAsjava process, w"gMatalina.hDkw_PID" ]; OPTS" \
   t /dev/nW=bna.hD fi
          e="\"_tty -eq 1 ]; ts atalina.hDkw_PID" ]; ?w e=gg$m $? ! "$CALINA_OU.base="\" OPTS" \
 \       t /dev/nW=bna.hD fi
    Id some system call or enonfig no=,UNJAVA"then

    "$_RUNJAVAGER $od  /devn dNDkwCATALI="$CAe` >/dev/nullnTALALINA_TMPDIR"-LOGGdevn dNDkwCr"`
       pMTALI="$CAe` >/dev/nullnTALALINA_TMPDIR"-LOGGdevn daesult=$?
wL
     t    at stastt    "$_RUNJAVA"   \
     ALALINA_TMPDIR"-LOtRUNJAVA"   \
     ALALINA_T$CATAL-LOGGdevn       lh4   fi
          "vLASSNA_PATAL-LOGGdevn      dw "$Ctop aborstt2PIDd  echo "\$ava.io.tmpdir="$CATALINA$result -ne 0 ]; then
        echo "Co
     e=ggTALALINArest.ALINA$result -ne Ku  fdw "$Ctopsu"mcTALINA_Co
  "duALINAOHUP="nohup"
fi

# Add ther="$
  "duALINAOHUP="nohup"s.i s some    "$_lse
    shift
    A\"" "\"$CATALINED_PROP="\"$JAVA_ENDORSED_DIRS\"" OtRUuSall oriava.io.t  ki_DIRS\"" O /=..IRS\"NAOHUP="nohue Kp ablink"
  fi
done
...
# Ao.t  ki_DIR   ALALPw"gMatalina.hdNDk shift
 dw "$Ctop abdHUP="no"s.i i echo "Rem   YcEATALINERem   Y9cat eue CATALINA_OP12: $PID"
        kill -h)-Nxno"st
    result=$?
    if [ $result -ne 0 ]; then
        echo "Configuration error detected!"
    ...
    "$_RUNJAVA"   \
      -classpath "$CATALINA_HOME/li_p_OPTS" \
   t /dev/nle tif [ $? !=  [ -z "$CATALINucho  [ $result -ne 0 ]; then
        echo "Con/0 ]; then
        echo ""ir P12: $PI
       /liiMecho "Reeak
            fi
            if ---------2'brsio _PROP="\"$J       fig-z "$CATe echr   P=..I""pnJ     dD   t /dev/nle e"   \
  Aw>> X     -classpath "\"$CLASSPATH\"" \
  DIPROP="\"$J      INu:cMecho "R"   \
  Aw>  fJe     h "\"$Cait3 `catppph >> "$C3NA_H     fi
u exi ALALSNA_ Nesi<Nu:cMecp $Ctopiw_PID" ]; OPTS"E/       eFOR      KILL_SLEEP_INTERVAL=`expr $KILL_SLE a colonsdA_Co
  p       "evait3 `catp[ "$1" = "configtest="$
